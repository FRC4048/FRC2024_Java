...
public class Drive extends Command {
    ...
    public void execute() {
        ...
        double fwd = MathUtil.applyDeadband(fwdSupplier.getAsDouble(), deadzone) * Constants.MAX_VELOCITY;
        double str = MathUtil.applyDeadband(strSupplier.getAsDouble(), deadzone) * Constants.MAX_VELOCITY;
        ChassisSpeeds driveStates;
        if (alignable == null){
            double rcw = MathUtil.applyDeadband(rtSupplier.getAsDouble(), deadzone) * Constants.MAX_VELOCITY;
