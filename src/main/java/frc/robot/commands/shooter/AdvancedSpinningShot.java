...
public class AdvancedSpinningShot extends Command {
    ...
    public AdvancedSpinningShot(Shooter shooter, Supplier<Pose2d> curentPoseSupplier) {
    ...
    public boolean isFinished() {
        return timer.hasElapsed(Constants.ADVANCED_SPINNING_SHOT_TIMEOUT);
    }

    private ShooterSpeed calcuateShooterSpeed() {
        Pose2d currentPose = pose2dSupplier.get();
        if (currentPose.getY() > Constants.SPEAKER_TOP_EDGE_Y_POS) return new ShooterSpeed(Constants.SHOOTER_MOTOR_HIGH_SPEED, Constants.SHOOTER_MOTOR_LOW_SPEED);
        else return new ShooterSpeed(Constants.SHOOTER_MOTOR_LOW_SPEED, Constants.SHOOTER_MOTOR_LOW_SPEED);
    }
